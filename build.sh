...
make_image() {
    ...
    mkfs.ext4 -U $BOOT_UUID -L mageia_boot -b 4096 images/$image_name/boot.img

    ###### create btrfs filesystem on root.img ######
    echo '### Creating btrfs filesystem on root.img '
    mkfs.btrfs -U $BTRFS_UUID -L 'mageia_asahi' $image_dir/$image_name/root.img
    ...
    sed -i "s/BOOT_UUID_PLACEHOLDER/$BOOT_UUID/" $image_mnt/boot/efi/EFI/mageia/grub.cfg
